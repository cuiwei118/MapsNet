...
def Nest_Net2(num_class=1, deep_supervision=True):
    nb_filter = [32, 64, 128, 256, 512]
    # nb_filter = [16, 32, 64, 128, 256]
    mode = 'residual'  # mode='residual' seems to improve better than DS
    # Handle Dimension Ordering for different backends
    bn_axis = 3
    inputs1 = Input(shape=(512,512,3))
    inputs2 = Input(shape=(512,512,3))
    inputs = concatenate([inputs1,inputs2],axis=bn_axis)
    ...
    if deep_supervision:
        model = Model(inputs=[inputs1,inputs2], outputs=[nestnet_output_1,
                                            nestnet_output_2,
                                            nestnet_output_3,
